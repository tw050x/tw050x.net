    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (stat.isDirectory()) {
          ...
          const methodName = entry.replace(/\.(ts|tsx|js|jsx)$/, '').toLowerCase();
          if (validMethods.includes(methodName)) {
            try {
              ...
              // Recursively unwrap all nested default objects
              let middleware = middlewareModule;
              while (middleware && typeof middleware === 'object' && 'default' in middleware) {
