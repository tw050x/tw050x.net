...
import { defineService } from "@tw050x.net/service-library";
...
defineService({
  ...
  onPrepare: async (service) => {
    await Promise.all([
      service.configuration.use('administration.service.allowed-origins'),
      service.configuration.use('administration.service.host'),
