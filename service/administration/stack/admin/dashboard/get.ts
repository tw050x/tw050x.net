...
export default defineServiceMiddleware([
  ...
  async (context) => {
    ...
    if (context.incomingMessage.accessTokenCookie.authorised === false) {
      return void sendUnauthorizedHTMLResponse(context.serverResponse, forbiddenDocument());
    }
  ...
  async (context) => {
    return void sendOKHTMLResponse(context.serverResponse, dashboardDocument());
  }
])
